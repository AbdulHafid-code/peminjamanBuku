...
@section('title')
	Dashboard Admin | Edit Transaksi
@endsection

@section('content')
	<div class="title-container">
		<div>
			<h1 class="title">Edit Transaksi Peminjaman Buku</h1>
    ...
    <form method="POST" action="{{ route('transaksi.update', $transaksi->id_transaksi)}}" class="w-full rounded-lg bg-white dark:bg-gray-800/50 shadow-md shadow-gray-200/60 dark:shadow-violet-800/20 p-3 sm:p-6 space-y-4 text-gray-600 dark:text-gray-400">
        @method('PUT')
        @csrf
        ...
        <div class="space-y-1">
            ...
            <select id="bukuSelect" name="buku_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500">
                @foreach ($buku as $item)
                    <option value="{{ $item->id_buku }}" data-stok="{{ $item->stok }}" {{ old('buku_id', $transaksi->buku_id) == $item->id_buku ? 'selected' : '' }}>
        ...
        <div class="space-y-1">
            ...
            <select name="user_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500">
                @foreach ($user as $item)
                    <option value="{{ $item->id_user }}"  {{ old('user_id', $transaksi->user_id) == $item->id_user ? 'selected' : '' }}>
        ...
        <div class="space-y-1">
            ...
            <input type="date" value="{{old('tanggal_pinjam', $transaksi->tanggal_pinjam)}}" name="tanggal_pinjam" placeholder="Masukkan Tanggal Pinjam" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500"/>
        </div>
        ...
        <div class="space-y-1">
            ...
            <input type="date" value="{{old('tanggal_kembali', $transaksi->tanggal_kembali)}}" name="tanggal_kembali" placeholder="Masukkan Tanggal Kembali..." class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500"/>
        </div>
